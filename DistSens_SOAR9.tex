...
\begin{frame}{Motivation: Why Bio-Inspired Distributed Sensing?}
  ...
  \movie[externalviewer]{%
	  {\centering\color{blue}Amazing seagull!!!}}
...
\begin{frame}{Motivation: Why Bio-Inspired Distributed Sensing?}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
	      \item<1->{Current UAV autopilot technologies}\\[2.5em]
	      \item<3->{Challenges}\\[2.5em]
	      \item<5->{Potential use of force and flow information}\\[2.5em]
      \end{itemize}
    \end{column}
...
\begin{frame}{Previous Research at UoB: Strain sensing}
  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
		...
		\begin{column}{0.4\textwidth}
			\begin{itemize}
				\item<2-> {\movie[width=0.8\textwidth,externalviewer]{Open loop free flight}
					{./Videos/StrainSensingExperiment_037_SlowDownShort.mp4}}
				\item<3-> {\movie[width=0.8\textwidth,externalviewer]{Closed loop free flight}
    ...
    %\begin{column}{0.6\textwidth}
			...
			%\only<2>{
				%\begin{figure}[!htb]
	  			...
	  			%\input{./Figures/StrainSensorsInWing.tex}
	  			%\caption{Strain sensing platform instrumentation}
	  			%\label{Fig:StrainExpPlatformInst2}
				%\end{figure}
			%}
			%\only<4>{
        %\centering
