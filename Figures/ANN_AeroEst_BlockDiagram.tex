...
\begin{tikzpicture}[x=1.5cm, y=1.5cm, >=stealth]
  ...
  %\node [every neuron/.try, neuron \m/.try] (input-\m) at (0,2.5-\y) {};
	\node [every neuron/.try, neuron \m/.try] (input-\m) at (0,2.75-\y*1.5) {};
  % Hidden layer nodes
  \foreach \m [count=\y] in {1,2,3,missing,4}
  \node [every neuron/.try, neuron \m/.try ] (hidden-\m) at (3,3.85-\y*1.10) {};
  % Output layer nodes
...
%     \draw [ArrowObject1] (output-1) -- ++(2.5,0);
%     \draw [ArrowObject1] (output-2) -- ++(2.5,0);
%     \draw [ArrowObject1] (output-3) -- ++(2,0);
  \draw [ArrowObject1] (output-1) -- ++(2.5,0)
	  node[above, midway, text width=4em, align=center] {${\alpha \,\& \,V}$\\or\\${D, \,L \,\& \,M}$};
%   \draw [ArrowObject1] (output-2) -- ++(2.5,0)
% 	  node [above, midway] {};
  %\draw [ArrowObject1] (output-3) -- ++(2,0)
